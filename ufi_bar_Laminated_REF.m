function [y] = ufi_bar_Laminated_REF(x,rot,A,B,D,A55,Neutral_R,K_R,K_R_limit)
...
y = C2*H1*sinh(alpha*X)+C4*H2*sinh(beta*X)-C5*(K_R*Neutral_R^2+A(1,1))/(B(1,1)+Neutral_R*A(1,1))*X;
...
y = (C2*alpha*H3-C4*beta*H4)*sinh(alpha*X)*cos(beta*X)+(C4*alpha*H3+C2*beta*H4)*cosh(alpha*X)*sin(beta*X)-...
    C5*(K_R*Neutral_R^2+A(1,1))/(B(1,1)+Neutral_R*A(1,1))*X;
