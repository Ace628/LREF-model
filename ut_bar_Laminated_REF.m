function [y] = ut_bar_Laminated_REF(x,rot,A,B,D,A55,Neutral_R,K_R,K_R_limit)
...
y = C2*Q1*sinh(alpha*X)+C4*Q2*sinh(beta*X)-C5*(K_R*Neutral_R^3+Neutral_R*A(1,1))/(B(1,1)+Neutral_R*A(1,1))*X;
...
y = (C2*alpha*Q3+C4*beta*Q4)*sinh(alpha*X)*cos(beta*X)+(C4*alpha*Q3-C2*beta*Q4)*cosh(alpha*X)*sin(beta*X)-...
    C5*(K_R*Neutral_R^3+Neutral_R*A(1,1))/(B(1,1)+Neutral_R*A(1,1))*X;
